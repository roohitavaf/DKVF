...
public class ChannelManager implements Runnable {
	 ...
	 * @param tryAgainWaitTime
	 *            The time before trying again in case of a failed delivery
	 * @param capacity
	 *            The capacity of pending messages
	 * @param logger
	 *            The logger
	 */
	 ...
	 * 
	 * @param sm
	 *            The ServerMessage object to add to the queue.
	 */
	public void addMessage(ServerMessage sm) {
		//for simulation
		if (!isBlocked)
			deque.addLast(sm);
		//otherwise it does not send it! (it is not for simulation. If you set delay to 0, there is not problem).
	}
